...
def eval_model(args):
    ...
    for idx, source in enumerate(tqdm(questions)):
        ...
        # Compute GT superpoint mask for click prompt (ablation)
        pt_segment = torch.tensor(gt_mask.astype(np.float32))
        p2s_map = torch.tensor(superpoint_mask)
        sp_segment = scatter(pt_segment.float(), p2s_map.long(), reduce="mean", dim=-1)
        gt_sp_mask = sp_segment > 0.5
        ...
        with torch.inference_mode():
            pred_mask = model.generate(
                ...
                max_new_tokens=64,
                tokenizer=tokenizer,
                click_mask=[gt_sp_mask.to(device)],
                use_cache=True)
